...
class knifeDataset(Dataset):
    ...
    def build_transforms(self):
        image_size = (args.resized_img_height, args.resized_img_weight)
        if self.mode == "train":
            resize_transform = (
                T.RandomResizedCrop(
                    image_size,
                    scale=(args.crop_scale_min, 1.0),
                    ratio=(args.crop_ratio_min, args.crop_ratio_max),
                )
                if args.random_resized_crop
                else T.Resize(image_size)
            )
            ...
            transform_list = [
                ...
                T.RandomRotation(degrees=args.random_rotation) if args.random_rotation > 0 else None,
