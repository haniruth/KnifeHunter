...
def main():
    ...
    val_imlist = pd.read_csv(args.val_datacsv)
    ...
    scheduler_kwargs = lr_scheduler_kwargs(args)
    if args.lr_scheduler == "CosineAnnealingLR":
        scheduler_kwargs["T_max"] = args.epochs * len(train_loader)
    scheduler = init_lr_scheduler(optimizer, **scheduler_kwargs)
    ...
    for epoch in range(args.epochs):
        ...
        if val_metrics[0] > best_map:
            best_map = val_metrics[0]
            best_path = os.path.join(args.saved_checkpoint_path, f"Knife-{args.model_mode}-best.pth")
            torch.save(model.state_dict(), best_path)
            log.write(f"\nBest checkpoint: epoch={epoch + 1}, mAP={best_map:.4f}, path={best_path}")
